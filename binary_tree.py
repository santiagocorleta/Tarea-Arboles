...
class BinaryTree:
    ...
    class __nodeTree:
        ...
        def __init__(self, value: Any, other_values: Optional[Any] = None):
            self.value = value                                 #clave principal por la que se ordena
            self.other_values = other_values                   #datos adicionales  
            self.left = None
            self.right = None
            self.hight = 0                                       #altura del nodo, necesario para el balanceo
    ...
    def pre_order(self):                    #recorridos root-left-right
    ...
    def in_order(self):                        #recorrido left-root-right
        def __in_order(root):
            if root is not None:
    ...
    def post_order(self):                     #recorrido right-root-left
    ...
    def search(self, value: Any) -> __nodeTree:               #si el valor es menor va a un subarbol izquierda, si es mayor a un subarbol derecha
        def __search(root, value):   
            if root is not None:
                ...
                else:                                          #devuelve el nodo que coincide
    ...
    def proximity_search(self, value: Any) -> __nodeTree:        #recorre todo el arbol e imprime valoes que cominezzan con un prefijo
        def __search(root, value):
            if root is not None:
